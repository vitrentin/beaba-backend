...
export async function createFunction(app: FastifyInstance) {
  app.post("/function", { onRequest: [verifyJWT] }, async (request, reply) => {
    ...
    try {
      ...
      // Verificar se a função já existe somente se nome_modulo não estiver definido
      let functions = await prisma.funcao.findUnique({
      ...
      } else if (!isDefined(functionData.nome_modulo)) {
        return reply.code(400).send({ error: "Função já existe" });
      }
